...
export async function POST(req: NextRequest) {
  try {
    ...
    // For Messenger, we look for pageId in the config usually, 
    // but the schema has phoneNumberId. We might need a generic 'externalId' or check config.
    const channel = await prisma.channel.findFirst({
      where: {
        type: "FACEBOOK", // Or "MESSENGER" if we have that enum
        status: "CONNECTED",
        // We'll assume pageId is stored in config or phoneNumberId field for simplicity
        OR: [
    ...
    if (!channel || !channel.chatbot) {
      console.error(`No connected Messenger channel found for Page ID: ${pageId}`);
      return NextResponse.json({ ok: true });
    }
    ...
    // 6. Send Reply via Messenger API
    const config = channel.config as any;
    const accessToken = config.accessToken;

    if (accessToken) {
      await sendMessengerMessage(senderId, replyText, accessToken);
    }
